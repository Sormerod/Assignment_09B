...
# SOrmerod, 2020-Sep-07, Added code for select_cd
# SOrmerod, 2020-Sep-07, Added code for add_track
#------------------------------------------#
...
class DataProcessor:
    ...
    def select_cd(table: list, cd_idx: int) -> DC.CD:
        ...
        for row in table:
            if row.cd_id == cd_idx:
                return row
        raise Exception('This CD / Album ID does not exsist')
    ...
    def add_track(track_info: tuple, cd: DC.CD) -> None:
        ...
        """
        trkpos, trkttl, trklgth = track_info
        try:
            trkpos = int(trkpos)
        except:
            raise Exception('Position must be an Integer!')
        track = DC.Track(trkpos,trkttl, trklgth)
        cd.add_track(track)
